...
chrome.action.onClicked.addListener(async(tab) => {
  ...
  chrome.scripting.executeScript({
    ...
    func: (extractedClass, imageClass) => {
      ...
      const elementsToProcess = cells.concat(slides);
      if (elementsToProcess.length) {
        elementsToProcess.forEach(cell => {
          // If the element contains anchor(s) with href, prefer those hrefs as the image sources.
